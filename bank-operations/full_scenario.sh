...
for t in mouvements_valides mouvements_valides_auto_expunge mouvements_valides_sorting; do
	...
	for i in `seq 0 10`; do
		...
		~/nb -v run driver=cql yaml=mouvements tags=type:write dsehost=localhost table=$table cycles=$WRITE_ROWS
		sleep 10
		log "Start READ #$i with ${READ_ROWS} rows"
		~/nb -v run driver=cql yaml=mouvements tags=type:read dsehost=localhost table=$table cycles=$READ_ROWS
		sleep 10
		log "Start mixed #${i} with ${MIXED_ROWS} rows"
		~/nb -v run driver=cql yaml=mouvements tags=phase:main dsehost=localhost table=$table cycles=$MIXED_ROWS
	done
